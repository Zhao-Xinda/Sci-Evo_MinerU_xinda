...
**Status as of 2026-05-11:** 237 traces extracted from 237 OA total-synthesis papers; 92 % JSON-Schema validation pass; 64 % of traces contain at least one explicit failed-attempt step (>2× the 30 % expectation).
...
A structured corpus of **237** total-synthesis records ("traces"), each derived from one peer-reviewed Open Access chemistry paper. A trace is a single JSON document conforming to `schema/sci_evo_total_synthesis.schema.json` and contains:
...
- **Failure-rich.** 84% of traces contain explicit failed attempts disclosed by authors (target was 30 %).
- **Reasoning-rich.** 20.1 steps per trace on average, each carrying a `[Background] / [Gap] / [Decision]` triple — direct training signal for chain-of-thought / agent-style models.
...
## 5. Quality Metrics (pilot, N=237)

| Metric | Value |
|---|---|
| Trace extraction success | **237/250 (100 %)** |
| JSON-Schema pass rate | **90%** |
| Average steps per trace | **20.1** |
| Median steps per trace | 19 |
| Max steps per trace | 78 |
| Traces with ≥1 failed_attempt | **199** (84%, target ≥ 30 %) |
| Total failed_attempts disclosed | **966** |
| Total revision steps | **566** |
| `target.smiles` filled | **87%** |
| SMILES parseable rate (RDKit) | 71% |
| `evidence.text_span` coverage | **99%** |
| Convergent-strategy traces | 63% |
| Total SMILES extracted | 10899 |
...
This dataset construction was carried out in 2026. As of submission the corpus content draws on OA chemistry papers indexed by Europe PMC up to 2026-05-11; new OA papers continue to be added on an ongoing basis through the same pipeline.
